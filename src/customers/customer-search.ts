 ...
 * `name`, its `externalRef` or ANY of its `contactEmails`. An empty/whitespace term matches
...
export function matchesCustomerSearch(customer: Customer, term: string): boolean {
  ...
  const haystacks = [customer.name ?? '', customer.externalRef, ...customer.contactEmails];
